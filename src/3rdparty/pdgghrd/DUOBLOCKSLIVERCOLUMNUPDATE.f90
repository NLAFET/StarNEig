subroutine duoblockslivercolumnupdate( a, desca, b, descb, IA, m, rowblksz, numgroups, groupinfo, trafs, info )

  ! TODO Keep within 132 columns.
...
!              dimension LLD_B x LOCc(N_B)
!
!              The distributed matrix B.
!     
...
!     
!              The row location of sub( A ) in A.
!
...
!
  ...
  allocate(leftbuf(ldbuf, 2*mb), rightbuf(ldbuf, 2*mb))

  ! Localize the row range IA : IA + M - 1 into locrow1 : locrow2.
  call grn2lrn(IA, IA + m - 1, mb, nprow, myrow, descA(rsrc_), locrow1, locrow2)
  ...
  do
     ...
     if ( colsright .gt. 0 ) then
        call dgesd2d( ictxt, slivers(myleftsliver)%height, colsright, A(slivers(myleftsliver)%r1l + & 
             ( c2l - 1 ) * lldA), lldA, myrow, left )
        call dgesd2d( ictxt, slivers(myleftsliver)%height, colsright, B(slivers(myleftsliver)%r1l + & 
             ( c2l - 1 ) * lldA), lldB, myrow, left )
     end if
     ...
     if ( colsright .gt. 0 ) then
        call dgesd2d( ictxt, slivers(myrightsliver)%height, colsleft, A(slivers(myrightsliver)%r1l  + & 
             ( c1l - 1 ) * lldA), lldA, myrow, right )
        call dgesd2d( ictxt, slivers(myrightsliver)%height, colsleft, B(slivers(myrightsliver)%r1l  + & 
             ( c1l - 1 ) * lldB), lldB, myrow, right )
     end if

     ! Step 3a (right cross-border): Receive A.
     if ( colsright .gt. 0 ) then
        call dgerv2d( ictxt, slivers(myrightsliver)%height, colsright , rightbuf(1, 1 + colsleft) , ldbuf, myrow, right )
     end if
     
     ! Step 3b (right cross-border): copy my part of A to rightbuf
     call dlacpy( 'A', slivers(myrightsliver)%height, colsleft, A(slivers(myrightsliver)%r1l + ( c1l  - 1 ) * lldA ),  &
          lldA, rightbuf, ldbuf ) 
     
     ! Step 4a (right cross-border): Update A.
     call dgemm( 'N', 'N', slivers(myrightsliver)%height, colsleft, numcols, one, rightbuf, ldbuf, & 
          trafs( trafptr ), ldtraf, zero, A(slivers(myrightsliver)%r1l + ( c1l - 1 ) * lldA ), lldA ) 

     ! Step 3c (right cross-border): Receive B.
     if ( colsright .gt. 0 ) then
        call dgerv2d( ictxt, slivers(myrightsliver)%height, colsright, rightbuf(1, 1 + colsleft) , ldbuf, myrow, right )
     end if
     ! Step 3d (right cross-border): copy my part of B to rightbuf
     call dlacpy( 'A', slivers(myrightsliver)%height, colsleft, B(slivers(myrightsliver)%r1l + ( c1l  - 1 ) * lldB ), & 
          lldB, rightbuf, ldbuf ) 
     
     ! Step 4b (right cross-border): Update B.
     call dgemm( 'N', 'N', slivers(myrightsliver)%height, colsleft , numcols, one, rightbuf, ldbuf, & 
          trafs( trafptr ), ldtraf, zero, B(slivers(myrightsliver)%r1l + ( c1l - 1 ) * lldB ), lldB ) 
     
     ! Step 6 (left cross-border): Receive, update.
     500 continue
     ...
     if ( colsright .gt. 0 ) then           
        ...
        call dgemm( 'N', 'N', slivers(myleftsliver)%height, colsright, numcols, one, leftbuf, ldbuf, & 
             trafs( trafptr + colsleft * ldtraf ), ldtraf, zero, A(slivers(myleftsliver)%r1l  + ( c2l  - 1 ) * lldA ), lldA )     
        ! Receive B.
        call dgerv2d(ictxt, slivers(myleftsliver)%height, colsleft, leftbuf, ldbuf, myrow, left ) 
        ! Copy my part of B to leftbuf.
        call dlacpy( 'A', slivers(myleftsliver)%height, colsright, B(slivers(myleftsliver)%r1l + ( c2l - 1 ) * lldB ), & 
             lldB, leftbuf( 1, 1 + colsleft ), ldbuf )     
        ! Update B.
        call dgemm( 'N', 'N', slivers(myleftsliver)%height, colsright, numcols, one, leftbuf, ldbuf, & 
             trafs( trafptr + colsleft * ldtraf ), ldtraf, zero, B(slivers(myleftsliver)%r1l  + ( c2l  - 1 ) * lldB ), lldA )     
     end if
     ! Step 7 (local): Update.
     700 continue
...
end subroutine duoblockslivercolumnupdate
